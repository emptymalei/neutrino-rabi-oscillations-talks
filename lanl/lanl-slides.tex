...
0 &  B_{\{n_i\}} e^{i  {\color{red} \sum_a n_a k_a } x}\\
  B_{\{n_i\}}^* e^{ -i  {\color{red} \sum_a n_a k_a } x} & 0
\end{pmatrix}
\end{equation*}
...
where 
\begin{align*}
    B_{\{n_i\}} &= -(-i)^{\sum_a n_a} \tan 2\theta_m \left( \sum_a n_a k_a \right) \left( \prod_a J_{n_a}\left( \frac{A_a}{k_a}\cos 2\theta_m \right) \right),\\
    \Phi_{\{n_i\}} &= e^{i\left( \sum_a n_a \phi_a \right)}.
\end{align*}
...
% \begin{textblock*}{100pt}(270pt,18pt)
% \textblockcolour{white}
% \small
...
Consider the resonance mode ($k=\omega_{\mathrm m}$) and one other mode

\begin{equation*}
\mathbf{\widetilde H}\sim -\frac{\omega_{\mathrm m}}{2}\sigma_3 + \frac{1}{2} \left( \begin{pmatrix}
0 &  B_{1} e^{i   k x}\\
  B_{1}^* e^{ -i   k x} & 0
\end{pmatrix} + \begin{pmatrix}
0 &  B_{n} e^{i   n k  x}\\
...
\vspace{40pt}
\begin{equation*}
D' =  \left\vert \frac{B_2^2}{2 B_n(\omega_{\mathrm m}-n k)} \right\vert
\end{equation*}
