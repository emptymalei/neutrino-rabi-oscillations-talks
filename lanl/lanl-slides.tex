...
\begin{textblock*}{10pt}(225pt,1pt)
\small
\begin{equation*}
\begin{pmatrix}
\psi_e\\
...
\begin{textblock*}{70pt}(250pt,250pt)
\small
Low density
\end{textblock*}
...
\begin{align*}
    &n_e(x) = n_0 + \delta n \cos(k x) \\
    \Rightarrow & \lambda(x) = \lambda_0 + \delta \lambda \cos(k x )
\end{align*}
\end{tcolorbox}
...
\small
\begin{equation*}
\alpha = \frac{\sin2\theta_{\mathrm m}}{2}
\end{equation*}
...
\item
Oscillations with two driving fields of different frequencies: large width to destroy the resonance.
\end{enumerate}
