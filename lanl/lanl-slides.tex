...
\begin{equation*}
    \lambda(x) = \lambda_0 + \delta \lambda \cos(k x )
\end{equation*}
...
\caption*{Stimulated oscillations. $\lambda(x) = \lambda_0 +  A \cos (k x)$  with $\hat x = \omega_{\mathrm m} x $, $A=0.1\omega_{\mathrm m}$, $k=0.995\omega_{\mathrm m}$, $\theta_{\mathrm{m}}=\pi/6$}
\end{figure}
...
0 & \frac{1}{2}  B_n e^{i  {\color{blue} (n k) } x}\\
\frac{1}{2}  B_n^* e^{ -i  {\color{blue} (n k) } x} & 0
\end{pmatrix}
\end{equation*}
